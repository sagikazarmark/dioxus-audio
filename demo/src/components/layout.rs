//! Responsive shell shared by all demo routes.
        ...
        header { class: "sticky top-0 z-20 border-b border-base-300 bg-base-100/90 backdrop-blur",
            div { class: "mx-auto flex min-h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 sm:px-6",
                Link {
                    ...
                    span { class: "truncate text-sm font-semibold tracking-tight", "{name}" }
                }
                div { class: "flex shrink-0 items-center gap-2",
                    a {
                        ...
                        aria_label: "View {name} on GitHub",
...
pub fn PageHeader(
    ...
    let document_title = format!("{title} | dioxus-audio");
...
#[component]
pub fn SidebarNavLink<R>(route: R, #[props(into)] label: String) -> Element
where
    R: Routable + PartialEq,
{
    rsx! {
